...
MODEL_PATH   = DIR / "kokoro-v1.0.int8.onnx"       # ~88 MB â€” smallest full-quality build
...
SENTENCES = [
    ...
    "Fingerprints on the light switch â€” something touched that, and it was not alive.",
...
def download_models() -> None:
    ...
    console.print("[yellow]Model files not found â€” downloading nowâ€¦[/yellow]")
    for path, url in missing.items():
        console.print(f"  [dim]{path.name}[/dim]  â† {url}")
    ...
    console.print("[green]âœ“[/green] Downloads complete.\n")
...
def build_voice_table() -> Table:
    ...
    table.add_column("ðŸ‡¬ðŸ‡§  British", style="cyan", no_wrap=True)
    table.add_column("ðŸ‡ºðŸ‡¸  American", style="green", no_wrap=True)
...
def main() -> None:
    console.print(Panel(
        ...
        "[dim]Powered by Kokoro ONNX Â· Phasmophobia Edition[/dim]",
        border_style="cyan",
        expand=False,
    ))
    console.print()
    ...
    console.print("Loading voice modelâ€¦", style="dim")
    kokoro = Kokoro(str(MODEL_PATH), str(VOICES_PATH))
    console.print("[green]âœ“[/green] Model ready.\n")

    console.print(build_voice_table())
    console.print()
    console.print("[dim]Type a voice name to hear it Â· [bold]q[/bold] to quit[/dim]\n")
    ...
    while True:
        ...
        if choice not in ALL_VOICES:
            console.print(
                f"[yellow]Unknown voice:[/yellow] [bold]{choice}[/bold] â€” "
