"""Deterministic command parser — no LLM needed.

Parses player input into structured intents using regex/keyword matching.
Handles ~90% of inputs deterministically with zero latency.
Returns action="unknown" for unrecognized commands.
"""
...
# ── Guess / lock-in patterns ────────────────────────────────────────────────

GUESS_PATTERNS: list[re.Pattern] = [
    re.compile(r"\b(?:we\s+)?think\s+it(?:'s|'s|\s+is)\s+(?:a\s+)?(.+)", re.IGNORECASE),
    re.compile(r"\b(?:my|our)\s+guess\s+is\s+(?:a\s+)?(.+)", re.IGNORECASE),
    re.compile(r"\bguess(?:ing)?\s+(?:a\s+)?(.+)", re.IGNORECASE),
    re.compile(r"\bbet(?:ting)?\s+(?:on\s+)?(?:a\s+)?(.+)", re.IGNORECASE),
]

LOCK_IN_PATTERNS: list[re.Pattern] = [
    re.compile(r"\block(?:ing)?\s+(?:it\s+)?in\s+(?:on\s+)?(?:a\s+)?(.+)", re.IGNORECASE),
    re.compile(r"\block\s+in\s*$", re.IGNORECASE),  # "lock in" with no ghost (use current guess)
    re.compile(r"\bfinal\s+answer\s+(?:is\s+)?(?:a\s+)?(.+)", re.IGNORECASE),
]

# ── Journal / ghost card patterns ───────────────────────────────────────────

JOURNAL_PATTERNS: list[re.Pattern] = [
    re.compile(r"\b(?:what\s+(?:does?\s+)?(?:the\s+)?journal\s+say\s+about)\s+(?:the\s+)?(.+)", re.IGNORECASE),
    re.compile(r"\btell\s+(?:me|us)\s+about\s+(?:the\s+)?(.+)", re.IGNORECASE),
    re.compile(r"\b(?:everything|all)\s+(?:on|about)\s+(?:the\s+)?(.+)", re.IGNORECASE),
]

# ── Ghost test result patterns ──────────────────────────────────────────────

GHOST_TEST_RESULT_PATTERNS: list[re.Pattern] = [
    re.compile(r"\b(\w+)\s+test\s+(?:passed|confirmed|positive|yes)\b", re.IGNORECASE),
    re.compile(r"\b(\w+)\s+test\s+(?:failed|negative|no|disproved)\b", re.IGNORECASE),
    re.compile(r"\btest\s+(?:for\s+)?(\w+)\s+(?:passed|confirmed)\b", re.IGNORECASE),
    re.compile(r"\btest\s+(?:for\s+)?(\w+)\s+(?:failed|negative)\b", re.IGNORECASE),
]
...
def _apply_stt_corrections(text: str) -> str:
    """Apply known STT mishearing corrections."""
    result = text
...
def parse_intent(text: str) -> ParsedIntent:
    ...
    Returns action="unknown" with confidence=0.0 when no pattern matches.
    """
    text = text.strip()
    if not text:
        return ParsedIntent(action="unknown", raw_text=text, confidence=0.0)
    ...
    # 11. Guess — "we think it's a Deogen"
    ...
    # 13. Ghost test result — "Goryo test passed" / "Goryo test failed"
    for pattern in GHOST_TEST_RESULT_PATTERNS:
        m = pattern.search(text)
        if m:
            ghost_raw = m.group(1)
            ghost_name = _find_ghost_name(ghost_raw) or _find_ghost_name(text)
            passed = bool(re.search(r"\b(?:passed|confirmed|positive|yes)\b", text, re.IGNORECASE))
            if ghost_name:
                return ParsedIntent(
                    action="ghost_test_result",
                    ghost_name=ghost_name,
                    status="passed" if passed else "failed",
                    raw_text=text,
                )

    # 14. Journal / ghost card — "tell me about Banshee" / "everything on Wraith"
    for pattern in JOURNAL_PATTERNS:
        ...
        if m:
            ...
            ghost_name = _find_ghost_name(ghost_raw) or _find_ghost_name(text)
            if ghost_name:
                is_full = bool(re.search(r"\b(?:everything|all|full)\b", text, re.IGNORECASE))
                return ParsedIntent(
                    action="query_ghost_database",
                    ghost_name=ghost_name,
                    query_field="full" if is_full else "journal",
                    raw_text=text,
                )

    # 15. Ghost database query (generic)?
    ghost_name = _find_ghost_name(text)
    if ghost_name:
        return ParsedIntent(
            action="query_ghost_database",
